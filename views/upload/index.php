...
<div class="row">
	<div class="col-md-5">
		...
		<label>Type</label>
		<select name="type-paper" class="form-control">
			<option value="mid">Mid Semester exam</option>
			<option value="final">Final exam</option>
			<option value="repeat">Repeat exam</option>
			<option value="lab">Lab sessions</option>
	...
	<div class="col-md-5">
			<label>Semester</label>
		<select name="semester-paper" onchange="loadModulesPapers()" id="semesterPapers" class="form-control">
			<option value="1y1s">Select an option</option>
			<option value="1y1s">1st year 1st semester</option>
			<option value="1y2s">1st year 2nd semester</option>
			<option value="2y1s">2nd year 1st semester</option>
		...
		<script type="text/javascript">
			function loadModulesPapers(){
				for(var a=0;a<5;a++){
					var x = document.getElementById("modules-papers");
					x.remove(x.selectedIndex);
				}
				var e = document.getElementById('semesterPapers').selectedIndex;
				var select = document.getElementById('modules-papers');
				if(e == 0){
					var opt = document.createElement('option');
					opt.value = '-----';
					opt.innerHTML = '-------';
...
<div class="row">	
	<div class="col-md-5">
		<label>Module</label>
	<select name="module-paper" id="modules-papers" class="form-control">
		<option value=""> ------- </option>
	</select>
	</div>
	<div class="col-md-5">
	...
	<label>Year</label>
	<select name="year-paper" class="form-control">
		<option value="2014">2014</option>
		<option value="2013">2013</option>
		<option value="2012">2012</option>
		<option value="2011">2011</option>
...
<div class="row">
	<div class="col-md-7">
		<a class='btn btn-primary' href='javascript:;'>
			Choose File...
			<input type="file" style='position:absolute;z-index:2;top:0;left:0;filter: alpha(opacity=0);-ms-filter:"progid:DXImageTransform.Microsoft.Alpha(Opacity=0)";opacity:0;background-color:transparent;color:transparent;' name="file-paper" id="file" size="40"  onchange='$("#upload-file-info-papers").html($(this).val());'>
		</a>
		&nbsp;
		<span class='label label-info' id="upload-file-info-papers"></span>
		...
		<!--<input type="file" name="file" id="file"> -->
		<br/>
		<input type="submit" name="submit-paper" value="Submit" class="btn btn-success">
